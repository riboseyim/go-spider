...
func Build_USATraffic_HTML_ECharts(obj *Echarts_HTML_Obj) {
	...
	obj.Data = Build_USATraffic_ECharts_JS(dataMap, dataList, obj.xArrays, obj.DataSize)
	Drawing_HTML_Template(obj)
	log.Println("----- Build_USATraffic_HTML_ECharts() finish------")
}
...
func Build_USATraffic_ECharts_JS(dataMap map[string]map[int]string, dataList *list.List, USA_Traffic_Years []int, DataSize int) string {
	html_js := ""
	...
	for j := range USA_Traffic_Years {
		...
		country_array := make([]*USA_Traffic_Air, len(dataMap)+1)
		year_array_rownum := 0
		//log.Println("----- len(country_array):%s", len(dataMap))
		for e := dataList.Front(); e != nil; e = e.Next() {
			cell, ok := (e.Value).(*USA_Traffic_Air)
			if ok && cell != nil {
				if cell.Year == year_this {
					//log.Println("----- add [%d] *Element %s", year_array_rownum, cell)
